...
float  Target_Roll,Target_Pitch,Target_Yaw,Target_height = 0.8;//Target_Roll,Target_Pitch单位为 x/10=度
...
//wu:定点相关结构体
struct HOLD_POSITION_OUTPUT output_x,output_y;
...
float Height_PID(float height);//定高PID
void Hold_Position(void);//定点
void Hold_Height(float Height);//定高
void Obstacle_Avoidance(void);//避障
...
void Flow_Output_NewDis(struct HOLD_POSITION_OUTPUT* a,float output) 
{
  a->Dist_Output_buf[a->Dis_index] = output;
  a->Dis_index = ((a->Dis_index + 1) % DEQUE_SIZE);  
}

float Flow_Output_getAvg(struct HOLD_POSITION_OUTPUT* a)
{
  float sum = 0.0;
  int i;
  for(i=0; i<DEQUE_SIZE; i++) {
    sum += a->Dist_Output_buf[i];
  }
  return (sum / (float)DEQUE_SIZE);
}
/**************************实现函数********************************************
...
//--------------------相应的飞行模块PWM输出运算-------------------------------------
	switch(Quadrotor_Mode)//检测当前状态
	{
		...
		case Quad_Assignment:		Mode_Assignment();//任务模式
			break;		
		case Quad_Landing:			Mode_Landing(0);//降落模式
			break;		
...
void Initial_Quadrotor_Math(void){
	...
	pidInit(&Position_X_Hold, Config.Position_X_Hold_P,
							Config.Position_X_Hold_I,
							Config.Position_X_Hold_D);
	pidInit(&Position_X_Speed,Config.Position_X_Speed_P,
							Config.Position_X_Speed_I,
							Config.Position_X_Speed_D);
	pidInit(&Position_Y_Hold, Config.Position_Y_Hold_P,
							Config.Position_Y_Hold_I,
							Config.Position_Y_Hold_D);
	pidInit(&Position_Y_Speed,Config.Position_Y_Speed_P,
							Config.Position_Y_Speed_I,
							Config.Position_Y_Speed_D);
	...
	Target_height = Config.target_hight; //装载目标高度值
}
...
// 	//xiang:这行代码是我改的
// 	//pidSetTarget_Measure(&AutoHigh_THR, Target_hight ,MS5611_Altitude/100.0f);
...
unsigned char Read_Mode(void)
{
		if(PWM_Input_CH6 < (int16_t)(PWM_Input_Offset-100)){//CH6小于1400
				if(PWM_Input_CH5 < (int16_t)(PWM_Input_Offset-100))//CH5小于1400
						return Quad_Level_Lock;//平衡
				else if(PWM_Input_CH5 >(int16_t)(PWM_Input_Offset+100))//CH5大于1600
						return Quad_Take_Of;//定高模式
		}
		else if(PWM_Input_CH6 >(int16_t)(PWM_Input_Offset+100)){//CH6大于1600
				if(PWM_Input_CH5 < (int16_t)(PWM_Input_Offset-100))//CH5小于1400
						return 	Quad_Hold_Position ;//定点模式
				else if(PWM_Input_CH5 >(int16_t)(PWM_Input_Offset+100))//CH5大于1600
					return Quad_Landing;//降落模式
		}
		return Quad_Manual;//ÊÖ¶¯Ä£Ê½
}
/**************************实现函数********************************************
...
void Get_Tartget_RPY(void)
{
	...
	if(GCSControl_Leftward)		Target_Roll=-51;
	if(GCSControl_Rightward)	Target_Roll=+51;
	if(GCSControl_Forward)		Target_Pitch=+51;
	if(GCSControl_Backward)		Target_Pitch=-51;
	if(GCSControl_LeftRotate)	Target_Yaw-=0.5;
	if(GCSControl_RightRotate)	Target_Yaw+=0.5;
	#endif
}
...
/**************************实现函数********************************************
	*函数原型:	void Obstacle_Avoidance(void)
	*功　　能:  避障函数
*******************************************************************************/
void Obstacle_Avoidance(void)
{
	//WU:添加的避障测试函数，采用四个超声波XY避障
	if(distance_front<=0.40)
		Target_Position_Y = -500;
	else if(distance_back<=0.40)
		Target_Position_Y = +500;
	else
		Target_Position_Y = 0;
	
	if(distance_left<=0.40)
		Target_Position_X = -500;
	else if(distance_right<=0.40)
		Target_Position_X = +500;
	else
		Target_Position_X = 0;
}
...
/**************************实现函数********************************************
	*函数原型:	void Hold_Position(void)
	*功　　能:  定点函数
*******************************************************************************/
void Hold_Position(void)
{
	//wu:以下代码是我用光流模块写的悬停,以上注释部分为原GPS定点
	float Output = 0;
  ...
  last_call_us = now_time;
	...
	Roll_Pitch_Yaw_AnglePID( Target_Roll/10.0f , Target_Pitch/10.0f , Target_Yaw);
	//以下部分为定点(XY轴)
	pidSet(&Position_X_Hold ,0.7, 0, 0.0);
	pidSet(&Position_X_Speed,2.8, 0.0, 0);
	pidSet(&Position_Y_Hold, 0.7, 0, 0.0);
	pidSet(&Position_Y_Speed, 2.8, 0.0, 0);
	if (PWM_Input_CH3 > (int16_t)(MINTHROTTLE + (MAXTHROTTLE - MINTHROTTLE) / 10))
	{ //遥控器油门大于10% 定点才会起作用，防止误动作引起电机转动
		if((Target_Roll > (int16_t)-50)&&(Target_Roll < (int16_t)60))
		{
		  pidSetTarget_Measure(&Position_X_Hold,Target_Position_X,Xmove);
			Position_X_MoveErr = Position_X_MoveErr
			+ (Interval_dt / (31.8310e-3 + Interval_dt)) * (Math_fConstrain(Position_X_Hold.merror , -1000.0f , 1000.0f) - Position_X_MoveErr);
			Position_SpeedTarget = pidUpdate_err(&Position_X_Hold,Position_X_MoveErr,Interval_dt);
			Position_SpeedTarget = Math_fConstrain(Position_SpeedTarget,-1500,+1500);
			pidSetTarget_Measure(&Position_X_Speed,Position_SpeedTarget,X_Speed);
			Position_X_SpeedErr = Position_X_SpeedErr
			+ (Interval_dt / (31.8310e-3 + Interval_dt)) * (Math_fConstrain(Position_X_Speed.merror , -1500.0f , 1500.0f) - Position_X_SpeedErr);
			Output = pidUpdate_err(&Position_X_Speed,Position_X_SpeedErr,Interval_dt)/10.0;
			Output = Math_fConstrain(Output,-60,60);
			Flow_Output_NewDis(&output_x,Output);
			Output=Flow_Output_getAvg(&output_x);
			RollRate.PID_out = RollRate.PID_out - Output;
		}
		else
		{
			Target_Position_X=0;
		...
		if((Target_Pitch > (int16_t)-60)&&(Target_Pitch < (int16_t)60))
		{
			pidSetTarget_Measure(&Position_Y_Hold,Target_Position_Y,Ymove);
			Position_Y_MoveErr = Position_Y_MoveErr
			+ (Interval_dt / (31.8310e-3 + Interval_dt)) * (Math_fConstrain(Position_Y_Hold.merror , -1000.0f , 1000.0f) - Position_Y_MoveErr);
			...
			Position_Y_SpeedErr = Position_Y_SpeedErr
			  + (Interval_dt / (31.8310e-3 + Interval_dt)) * (Math_fConstrain(Position_Y_Speed.merror , -1500.0f , 1500.0f) - Position_Y_SpeedErr);
			Output = pidUpdate_err(&Position_Y_Speed,Position_Y_SpeedErr,Interval_dt)/10.0;
			Output = Math_fConstrain(Output,-60,60);
			Flow_Output_NewDis(&output_y,Output);
			Output=Flow_Output_getAvg(&output_y);
			PitchRate.PID_out = PitchRate.PID_out + Output;
		}
		else
		{
			Target_Position_X=0;
...
/**************************实现函数********************************************
	*函数原型:	void Hold_Hight(void)
	*功　　能:  定高函数
*******************************************************************************/
void Hold_Height(float Height)
{	
		Quad_THR = (int16_t)PWM_Input_CH3;
		if (PWM_Input_CH3 > (int16_t)(MINTHROTTLE + (MAXTHROTTLE - MINTHROTTLE) / 10)){ 
				Quad_THR += Height_PID(Height);
		}
		else
		{
				pidReset(&Climb);
...
void Mode_Assignment(void)
{
	...
	// 			case Command_Down:
	// 				Target_hight = GroundAltitude;
	// 				if(MS5611_Altitude <= Tatget_hight)
	// 				{
	// 					//Quad_THR += Auto_High_PID(MS5611_Altitude , 0);
	// 					pidReset(&Climb);
	// 				}else
	// 				{
	// 					Quad_THR += Auto_High_PID(Target_hight , 1);
	// 				}
	// 				//Target_Roll = 0;
	// 				//Target_Pitch = 0;
	// 				//Roll_Pitch_Yaw_AnglePID( Target_Roll/10.0f , Target_Pitch/10.0f , Target_Yaw);
...
void Mode_Landing(uint32_t flag)
{
    ...
    Hold_Position();
		if(altitude < 0.2f)
		{
				...
				Quad_THR += Z_Speed_PID(-0.05);
		}
		else{
				Quad_THR += Z_Speed_PID(-0.08);
		}
		THROTTLE = Quad_THR;
		PWM_Write_Motors();
}
...
void Mode_Take_Of(void)
{
	  float alt_d;
		...
		Hold_Height(Target_height);
//		if(flag == 0){
//				alt_d = MS5611BA_Get_D();
//				if(Abs((MS5611_Altitude/100.0f -Target_height), 0.1f) && Abs(alt_d, 0.5f)){
//						balance_thr = Quad_THR;
//						flag = 1;
//				}		
//		}
//		if(Land_End == 1){
