...
typedef struct
{
		//未积分的数据
		//记录总的创建的IIC的帧数
		uint16_t frame_count_sum;                           //uint16_t counts created I2C frames [frames]
		//X轴最新一帧所有像素移动和*10
		int16_t pixel_flow_x_sum;                            //int16_t latest x flow measurement in pixels*10 [pixels]
		//Y轴最新一帧所有像素移动和*10
		int16_t pixel_flow_y_sum;                            //int16_t latest y flow measurement in pixels*10 [pixels]
		//X轴速度
		int16_t flow_comp_m_x;                             //int16_t x velocity*1000 [meters/sec]
		//Y轴速度
		int16_t flow_comp_m_y;                             //int16_t y velocity*1000 [meters/sec]
		//光流图像质量
		int16_t qual;                                      //int16_t Optical flow quality / confidence [0: bad, 255: maximum quality]
		//X轴陀螺仪速度
		int16_t gyro_x_rate;                               //int16_t latest gyro x rate [rad/sec]
		//Y轴陀螺仪速度
		int16_t gyro_y_rate;                               //int16_t latest gyro y rate [rad/sec]
		//Z轴陀螺仪速度
		int16_t gyro_z_rate;                               //int16_t latest gyro z rate [rad/sec]
		//陀螺仪数据范围
		uint8_t gyro_range;                                 //uint8_t gyro range [0 .. 7] equals [50 deg/sec .. 2000 deg/sec]
		//超声波数据上次更新到现在的时间间隔
		uint8_t sonar_timestamp1;                          //uint8_t time since last sonar update [milliseconds] 
		//地面距离  正值：已知距离   负值：未知距离
		int16_t ground_distance1;                           //int16_t Ground distance in meters*1000 [meters]. Positive value: distance known. 
																												//Negative value: Unknown distance
		//积分后的数据
		//上次读取数据后，数据更新了多少次
		uint16_t frame_count_since_last_readout;             //uint16_t number of flow measurements since last I2C readout [frames]
		//上次读取IIC数据后，X轴速度积分后所得值
		int16_t pixel_flow_x_integral;                      //int16_t  accumulated flow in radians*10000 around x axis since last I2C readout [rad*10000]
		//上次读取IIC数据后，Y轴速度积分后所得值
		int16_t pixel_flow_y_integral;                      //int16_t  accumulated flow in radians*10000 around y axis since last I2C readout [rad*10000]
		//上次读取IIC数据后X轴角速度积分值
		int16_t gyro_x_rate_integral;                       //int16_t  accumulated gyro x rates in radians*10000 since last I2C readout [rad*10000]  
		//上次读取IIC数据后Y轴角速度积分值
		int16_t gyro_y_rate_integral;                       //int16_t  accumulated gyro y rates in radians*10000 since last I2C readout [rad*10000] 
		//上次读取IIC数据后Z轴角速度积分值
		int16_t gyro_z_rate_integral;                       //int16_t  accumulated gyro z rates in radians*10000 since last I2C readout [rad*10000]
		//上次和这次读取IIC数据的时间间隔
		uint32_t integral_timespan;                         //uint32_t accumulation timespan in microseconds since last I2C readout [microseconds]
		//超声波数据上次更新到现在的时间间隔
		uint32_t sonar_timestamp2;                          //uint32_t time since last sonar update [microseconds]
		//地面距离
		int16_t ground_distance12;                          //int16_t  Ground distance in meters*1000 [meters*1000]
		//陀螺仪温度
		int16_t gyro_temperature;                           //int16_t  Temperature * 100 in centi-degrees Celsius [degcelsius*100] 
		//光流积分数据质量
		uint8_t quality;                                    //uint8_t averaged quality of accumulated flow values [0:bad quality;255: max quality]
