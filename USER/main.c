...
int main(void)
{
	...
	Initial_UART4(115200L);//uart4用于和副控板通信
	IIC_Init();	 //初始化I2C接口 
	PWM_Input_Initial(); 
	PWM_Output_Initial();
	delay_ms(300);//等待器件上电
	...
	while(1)//主循环
	{
		...
		UART4_Get_Data();	//获取副控板传来的数据
		//更新PWM输出，以控制四轴
		if(Servo_Update_Req)//这个标志位控制PWM输出周期，由定时器中断置1,见PWM_Output.c
		{//PWM输出更新
			Quadrotor_Motor_Update(); //计算四个电机的转速 并输出 z
		...
		if((micros()-system_micrsecond)>upload_time)//单位us
		{
			...
			char string_to_send2[100]={0};
			sprintf(string_to_send1, "\r\nCH1:%d,CH2:%d,CH3:%d,CH4:%d,CH5:%d,CH6:%d\r\n",UART4_CH[0],UART4_CH[1],UART4_CH[2],UART4_CH[3],UART4_CH[4],UART4_CH[5]);
      UART1_Put_String((unsigned char *)string_to_send1);
//			UART1_Put_String((unsigned char *)UART4_buffer);
			//UART1_Put_String((unsigned char *)string_to_send1);
//			sprintf(string_to_send1, "\r\nTarget:%d Y_Speed:%d Ymove:%f \r\n",PID_PITCH,Y_Speed,Ymove);
//      UART1_Put_String((unsigned char *)string_to_send1);
//		  sprintf(string_to_send2, "\r\nTarget:%d X_Speed:%d Xmove:%f \r\n",PID_ROLL,X_Speed,Xmove);
...
//			sprintf(string_to_send1, "\r\n%f  %f  %f  %f\r\n",distance_front,distance_back,distance_left,distance_right);
//			UART1_Put_String((unsigned char *)string_to_send1); 
...
void GCS_GetCommand(unsigned char PC_comm)//xiang：注意：这个函数是针对自己写的上位机的，如果要用captain上位机，就用上面那个函数
{
	...
	switch(PC_comm)//检查命令标识
	{
		...
		case 0x12:     	UART1_ReportPositionAndSpeed(Target_Position_X,Target_Position_Y,Target_height,
																									X_Speed,Y_Speed,Xmove,Ymove,MS5611_Altitude/100.0f);  break;
		...
		case 0x36:			UART1_Return_PID(&Position_X_Hold, &Position_X_Speed);			break;
		case 0x37:			UART1_Return_PID(&Position_Y_Hold, &Position_Y_Speed);			break;
		...
		case 0x46: 				UART1_Get_PID(&Position_X_Hold, &Position_X_Speed);				break;
		case 0x47: 				UART1_Get_PID(&Position_Y_Hold, &Position_Y_Speed);				break;
		case 0x50:        UART1_Get_PositionTarget(); 					break;
		...
		break;
		case 0x61:		    UART1_Report_PWMInOut(ypr[0], ypr[1], ypr[2]);		    break;//将PWM输入和输出的脉宽值发送到PC
		...
		case HMC_calib_begin:	LSM303_Start_Calib();	break;     //启动磁力计标定
		case HMC_calib:		    LSM303_Save_Calib();	break;     //保存磁力计标定
		case 0xE4: 						PWM_Save_Offset(); break; 
		case 0x71:
